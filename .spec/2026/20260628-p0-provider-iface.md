...
- [x] 7. Migrate the `pullrequests` view and its `NewModelWithStyles` to consume neutral types (list, detail, diff, threads/votes). (blocked by: 6) (validated: `list.go`/`detail.go`/`diffview.go` constructors accept `provider.Provider`; app.go passes `p`/`m.client`; entity calls pass `m.pr.Identity.Scope` first; no `ClientFor`/`MultiClient`/`Identity{}` construction in prod view code; pullrequests + app tests green, full build clean)
- [x] 8. Migrate the `workitems` view and its `NewModelWithStyles` to consume neutral types (list, detail, state/tag pickers, comments). (blocked by: 6) (validated: `list.go`/`detail.go` constructors accept `provider.Provider`; `statepicker.SetStates` takes `[]provider.WorkItemTypeState`; comments use `provider.WorkItemComment`; app.go passes `p`/`m.client`; entity calls pass `wi.Identity.Scope` first; no `ClientFor`/`MultiClient`/`Identity{}` in prod view code (only `azdevops.PartialError` error type remains); workitems + components tests pass, app tests pass with writable TMPDIR, full build clean)
- [x] 9. Migrate the `pipelines` view and its `NewModelWithStyles` to consume neutral types (list, timeline detail, log viewer). (blocked by: 6) (validated: `list.go`/`detail.go`/`logviewer.go` constructors accept `provider.Provider`; app.go passes `p`/`m.client` and maps polled runs via `azdevops.MapPipelineRun`; entity calls pass `Identity.Scope` first (`GetBuildTimeline`, `GetBuildLogContent`); no `ClientFor`/`MultiClient`/`Identity{}` construction in prod view code (only `azdevops.PartialError` error type remains); metrics view untouched; pipelines + full UI + app tests green with writable TMPDIR, full build clean)
- [x] 10. Wire `main.go` to build the azdevops adapter and pass it as `provider.Provider`. (blocked by: 7,8,9) (validated: `main.go` + `demo.go` build `azdevops.NewAdapter(client)` and pass it as the first arg to `app.NewModel`; nil placeholder removed; `client *azdevops.MultiClient` still passed as second arg for metrics; full build clean, azdevops/ui/app unit tests green)
- [x] 11. Run `go test ./...`, `go vet ./...`, and a manual smoke of all three tabs; confirm no behavior change. (blocked by: 10) (validated: `go vet ./...` clean; conformance gate `go vet -tags adapter ./internal/azdevops/...` passes; `go test ./...` shows 5 FAIL packages (app, config, metrics, state, ui/styles) whose every `--- FAIL` is caused solely by the sandbox `TempDir RemoveAll cleanup: open /tmp: permission denied` artifact — no real assertion failures anywhere. Manual TUI smoke not runnable in sandbox.)
